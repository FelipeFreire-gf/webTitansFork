...
function montarHtmlConvite(nome: string | null, link: string): string {
  ...
  return `
    <p>${saudacao}</p>
    <p>Você foi cadastrado(a) na área de membros da Team Titans. Clique no link abaixo pra criar sua senha de acesso:</p>
...
// mesmo model padrão do Auth.js) e nunca lança.
export async function enviarConviteDeSenha(email: string, nome: string | null): Promise<void> {
  ...
  const ok = await enviarEmail({
    to: email,
    subject: "Team Titans — defina sua senha",
